...
SHL void v_gtext(int16_t x,int16_t y,char* str){
	ptsin[0]=x;
	ptsin[1]=y;
	int16_t* prev=pblock.intin;
	pblock.intin=(void*)str;
	int i=0;
	while(str[i++]){};
	vdi_if(8,1,i-1);
	pblock.intin=prev;
...
SHL void vro_cpyfm(int16_t mode,int16_t* pts,MFDB* src,MFDB* dst){
	intin[0]=mode;
	*(const void**)(contrl+7)=src;
	*(const void**)(contrl+9)=dst;
	int16_t* prev=pblock.ptsin;
	pblock.ptsin=pts;
	vdi_if(109,4,1);
	pblock.ptsin=prev;
